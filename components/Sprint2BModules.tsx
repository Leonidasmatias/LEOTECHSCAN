...
export function TelecomAIView() {
  ...
  return <section className="s2b-view"><div className="s2b-hero panel"><div><p className="eyebrow">TELECOM AI</p><h2>Consulta inteligente local</h2><p>Engine por regras sobre SQLite, preparada para futura IA generativa.</p></div><a href={`/api/export?type=telecom-ai-report&q=${encodeURIComponent(submitted)}`}>Exportar CSV</a></div><div className="s2b-chat panel"><div className="s2b-question"><input value={question} onChange={(event) => setQuestion(event.target.value)} /><button onClick={() => setSubmitted(question)}>Perguntar</button></div><div className="s2b-examples">{examples.map((item) => <button key={item} onClick={() => { setQuestion(item); setSubmitted(item); }}>{item}</button>)}</div>{error ? <p className="error">{error}</p> : null}<article><span>{data?.intent || "intent"}</span><strong>{data?.answer || "Carregando resposta..."}</strong></article><JsonRows rows={data?.rows} /></div></section>;
}
...
export function RolloutView() {
  ...
  return <section className="s2b-view"><div className="s2b-hero panel"><div><p className="eyebrow">ROLLOUT INTELLIGENCE</p><h2>Planejamento operacional</h2><p>Estimativa de novos sites, equipes, MOS, instalacoes e prioridade.</p></div><a href="/api/export?type=rollout-intelligence">Exportar CSV</a></div>{error ? <p className="error">{error}</p> : null}<div className="s2b-kpis"><article className="panel"><span>Municipios avaliados</span><strong>{nf.format(data?.summary.total || 0)}</strong></article><article className="panel danger"><span>Prioridade muito alta</span><strong>{nf.format(data?.summary.veryHigh || 0)}</strong></article></div><JsonRows rows={data?.national} /></section>;
}
...
export function MarketView() {
  ...
  return <section className="s2b-view"><div className="s2b-hero panel"><div><p className="eyebrow">MARKET INTELLIGENCE</p><h2>Participacao e comparativos</h2><p>TIM x VIVO, distribuicao tecnologica, ranking de operadoras e presenca territorial.</p></div><a href="/api/export?type=market-intelligence">Exportar CSV</a></div>{error ? <p className="error">{error}</p> : null}<div className="s2b-kpis">{(data?.operators || []).map((item) => <article className="panel" key={String(item.operator)}><span>{String(item.operator)}</span><strong>{nf.format(Number(item.records || 0))}</strong><small>{nf.format(Number(item.sites || 0))} sites</small></article>)}</div><JsonRows rows={data?.byUf} /><JsonRows rows={data?.byTechnology} /></section>;
}
