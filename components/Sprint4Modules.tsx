...
function V3Hero({ eyebrow, title, text, exportUrl, capabilityKey }: { eyebrow: string; title: string; text: string; exportUrl?: string; capabilityKey?: string }) {
  return <div className="s2b-hero panel"><div><p className="eyebrow">{eyebrow}</p><h2>{title} {capabilityKey ? <CapabilityBadge capabilityKey={capabilityKey} /> : null}</h2><p>{text}</p>{capabilityKey ? <CapabilityNote capabilityKey={capabilityKey} /> : null}</div>{exportUrl ? <a href={exportUrl}>Exportar CSV</a> : null}</div>;
}
...
export function DigitalTwinView() {
  ...
  return <section className="s2b-view"><V3Hero eyebrow="DIGITAL TWIN TELECOM" title="Ativo inteligente do site" text="Visao operacional e estrategica do site, baseada nos dados disponiveis no SQLite." exportUrl={`/api/export?type=digital-twin-sites&id=${submitted}`} capabilityKey="digital_twin" />
...
export function ScenarioPlannerView() {
  ...
  useEffect(() => { run(); }, []);
  return <section className="s2b-view"><V3Hero eyebrow="SCENARIO PLANNER" title="Simulador de impacto" text="Compara antes e depois para LTS, TCI, OPI e market share." exportUrl={`/api/export?type=scenario-planner&operadora=${form.operadora}&uf=${form.uf}&tecnologia=${form.tecnologia}&meta=${form.metaNovosSites}`} capabilityKey="scenario_planner" />
...
export function AdvancedGisView() {
  ...
  return <section className="s2b-view"><V3Hero eyebrow="ADVANCED GIS" title="Geointeligencia avancada" text="Raio configuravel, proximidade, clusters por operadora/tecnologia e camadas estrategicas." exportUrl={`/api/export?type=advanced-geointelligence&id=${submitted.siteId}&radiusKm=${submitted.radius}`} capabilityKey="advanced_gis" />
