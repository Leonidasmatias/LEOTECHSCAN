...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (kind === "telecom-ai-report") {
      ...
      const aiFile = path.join(exportDir, "telecom_ai_report.csv");
    ...
    if (kind === "rollout-intelligence" || kind === "municipios-prioritarios") {
      ...
      const out = path.join(exportDir, kind === "municipios-prioritarios" ? "municipios_prioritarios.csv" : "rollout_intelligence.csv");
    ...
    if (kind === "alert-center") {
      ...
      const out = path.join(exportDir, "alert_center.csv");
      await writeFile(out, csvRows(report), { encoding: "utf8" });
    ...
    if (kind === "market-intelligence") {
      ...
      const out = path.join(exportDir, "market_intelligence.csv");
      await writeFile(out, csvRows(report), { encoding: "utf8" });
    ...
    if (kind === "mission-control-csv") {
      ...
      const missionFile = path.join(exportDir, `mission-control-${stamp()}.csv`);
    ...
    if (kind === "site-csv" || kind === "site-pdf") {
      ...
      if (kind === "site-csv") {
        const siteFile = path.join(exportDir, `site-${site.site}-${stamp()}.csv`);
      ...
      return new NextResponse(pdf, { headers: { "Content-Type": "application/pdf", "Content-Disposition": `attachment; filename="site-${site.site}-${stamp()}.pdf"` } });
    }
    if (kind === "operator-intelligence" || kind === "tci-ranking") {
      ...
      return new NextResponse(body, { headers: { "Content-Type": "text/csv; charset=utf-8", "Content-Disposition": `attachment; filename="${filename}"`, "X-Export-Rows": String(report.length - 1), "X-Export-Path": filePath } });
    }
    if (kind === "executive") {
    ...
    const body = Readable.toWeb(createReadStream(filePath)) as ReadableStream;
    return new NextResponse(body, { headers: { "Content-Type": "text/csv; charset=utf-8", "Content-Disposition": `attachment; filename="${filename}"`, "X-Export-Rows": String(rows), "X-Export-Path": filePath } });
  } catch (error) {
