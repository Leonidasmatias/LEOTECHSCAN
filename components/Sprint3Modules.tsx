...
export function DataQualityView() {
  ...
  return <section className="s3-shell">
    <div className="s3-head"><div><p className="eyebrow">SPRINT 3 ENTERPRISE</p><h2>Qualidade Cadastral <CapabilityBadge capabilityKey="data_quality" /></h2><span>Completude, coordenadas, endereco, municipio, UF e tecnologia.</span><CapabilityNote capabilityKey="data_quality" /></div><a href="/api/export?type=qualidade-cadastral">Exportar CSV</a></div>
...
export function DuplicatesView() {
  ...
  return <section className="s3-shell">
    <div className="s3-head"><div><p className="eyebrow">DEDUPLICACAO SUGESTIVA</p><h2>Possiveis Duplicidades <CapabilityBadge capabilityKey="duplicate_detection" /></h2><span>Nenhum registro e excluido; os grupos abaixo sao apenas candidatos para auditoria.</span><CapabilityNote capabilityKey="duplicate_detection" /></div><a href="/api/export?type=possiveis-duplicidades">Exportar CSV</a></div>
    {error ? <div className="error">{error}</div> : null}
...
export function NationalTimelineView() {
  ...
  return <section className="s3-shell">
    <div className="s3-head"><div><p className="eyebrow">TIMELINE NACIONAL</p><h2>Evolucao da Base Consolidada <CapabilityBadge capabilityKey="national_timeline" /></h2><span>Importacoes, totais por operadora e municipios monitorados.</span><CapabilityNote capabilityKey="national_timeline" /></div><a href="/api/export?type=timeline-nacional">Exportar CSV</a></div>
    {error ? <div className="error">{error}</div> : null}
